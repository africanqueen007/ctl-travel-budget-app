export default async function handler(req, res) {
  ...
  const prompt = `You are a flight data API. Your only job is to return a single number. Find the typical price for a roundtrip business class flight from Manila (MNL) to ${destination}, departing on ${departureDateString} and returning on ${returnDateString}. Return only the estimated price in USD as a single number, without any text, currency symbols, or commas. For example: 2950.`;
  ...
  try {
    ...
    // Log the actual AI response for debugging purposes
    ...
    if (!isNaN(parsedPrice)) {
      ...
      // If parsing fails, throw an error that includes the problematic text.
